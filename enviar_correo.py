...
def enviarCorreoRevision(cantidadArchivos, patrimonio, fecha_corte):
	...
	try:
		...
		lista_nombres = (
			'INSERT_Cuentas_Faltantes_PAT-%s_FCORTE-%s' % (patrimonio, fecha_corte),
			'DELETE_clientes_duplicados_TC_PAT-%s_FCORTE-%s' % (patrimonio, fecha_corte),
		)
		...
		for nombre_archivo in range(0,int(cantidadArchivos)):
			...
			part = MIMEBase('application', "octet-stream")
			part.set_payload(open(archivo_path, "rb").read())
			nombre_archivo = '%s_%s.sql' % (str(i),lista_nombres[nombre_archivo])
			part.add_header('Content-Disposition', 'attachment' ,filename=nombre_archivo)
			msg.attach(part)
			i += 1
		...
		raise Exception("El segundo correo no pudo enviarse: %s" % (e))